  ...
  `;
    ...
    //todo modelViewMatrix 繞 Z 軸旋轉 squareRotation 的當前值。
    //todo 要實際製作動畫，我們需要添加隨時間改變 
    //todo squareRotation 值的代碼。 
    //todo 我們可以通過創建一個新變量來跟踪我們上次動畫的時間（then）來做到這一點，
    //todo 然後將以下代碼添加到 main 函數的末尾

    var then=0;
    //todo draw the scene repeatedly
    function render(now) {
...
//!!記得加deltaTime
  ...
  //todo drawScene() 函數以在繪製正方形時將當前旋轉應用於正方形。 
  //todo 平移到正方形的初始繪圖位置後，我們像這樣應用旋轉：
      mat4.rotate(modelViewMatrix,//todo 目標矩陣
                  modelViewMatrix,//todo 哪個矩陣旋轉
                  squareRotation, //todo 弧度旋轉的量
                  [0,0,1]);       //todo 繞軸旋轉    
...

  {
    ...
    gl.enableVertexAttribArray(
      ...
      //todo 此代碼使用 requestAnimationFrame 要求瀏覽器調用函數“渲染”每一幀。
      //todo requestAnimationFrame 以毫秒為單位傳遞頁面加載後的時間。 
      //todo 我們將其轉換為秒，然後從最後一次減去它以計算 deltaTime，它是自上一幀渲染以來的秒數。
      //todo 在 draw scene 結束時，我們添加代碼來更新 squareRotation。
